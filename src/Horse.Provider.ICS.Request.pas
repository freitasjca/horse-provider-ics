...
// literally instead of raising. This runs during request population on the
// worker thread, outside any handler's try/except, so an exception here would
// turn a client typo into a 500. Bytes that are not valid UTF-8 fall back to
// the raw text for the same reason. A + is a space, per
...
begin
  ...
  begin
    ...
    begin
      ...
      // both. This used to store the WHOLE "name=value" text as the key with an
      // empty value, so Req.ContentFields['name'] always returned ''.
      if LPair <> '' then
      begin
        LEqPos := Pos('=', LPair);
        if LEqPos > 0 then
